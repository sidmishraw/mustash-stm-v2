...
public class SimpleDriver2 {
   ...
   */
  private static final STMAction<Boolean> deposit(Transaction t, TVar<Integer> account,
      int amount) {
   ...
   */
  private static final STMAction<Boolean> withdraw(Transaction t, TVar<Integer> account,
      int amount) {
    return t.read(account).bind(bal -> t.write(account, bal - amount));
  }
  ...
  public static void main(String[] args) {
...

    {
      ...
      stm.exec(Transaction.builder().stm(stm).action(t -> t.read(account1).bind(acc1Bal -> {
        if (acc1Bal > 1000) {
          return withdraw(t, account1, 100).bind(b1 -> deposit(t, account2, 100)
              .bind(b2 -> new STMAction<Boolean>(() -> b1.booleanValue() && b2.booleanValue())));
        } else {
          return withdraw(t, account1, 500).bind(b1 -> deposit(t, account2, 500)
              .bind(b2 -> new STMAction<Boolean>(() -> b1.booleanValue() && b2.booleanValue())));
        }
      })).build());
